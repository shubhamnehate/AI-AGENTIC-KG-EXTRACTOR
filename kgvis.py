...
def main():
    ...
    if uploaded_file is not None:
        ...
        option = create_echarts_option(nodes, links)

        freeze_graph = st.checkbox("Freeze graph updates")
        if not freeze_graph:
            graph = st_echarts(options=option, height="600px", key="live_graph")
        else:
            graph = st_echarts(options=option, height="600px", key="frozen_graph")

        if st.button('Take Snapshot'):
            components.html(
                f"""
                <script>
                html2canvas(document.querySelector("#{graph._id}")).then(canvas => {{
